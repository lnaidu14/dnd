...
const onJoinCampaign = (campaignId) => {
  router.push(`/campaigns/${campaignId}/session`);
};
...
const deleteCampaign = async (id) => {
  try {
      ...
      toast.current.show({severity:'success', summary: 'Success', detail:'Successfully deleted campaign', life: 3000});
  } catch (err) {
      console.error(err)
      setError(err);
      toast.current.show({severity:'failure', summary: 'Failure', detail:'Failed to delete campaign', life: 3000});
  } finally {
      setLoading(false);
    }
  };
